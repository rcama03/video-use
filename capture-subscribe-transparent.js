...
const FPS = 30;
const DURATION = 8.5;
const TOTAL = Math.ceil(FPS * DURATION); // 255
const DIR = '/tmp/frames_sub_t';
...
(async () => {
  ...
  // Inject controlled time override
  html = html.replace(
    'var T=(ts-T0)/1000;',
  ...
  console.log('Encoding transparent WebM (VP9 + alpha) with audio…');
  execSync(
    `ffmpeg -y -framerate ${FPS} -i ${DIR}/f%05d.png -i /tmp/subscribe-audio.wav ` +
    `-c:v libvpx-vp9 -pix_fmt yuva420p -b:v 0 -crf 15 -threads 4 ` +
    `-c:a libopus -b:a 128k ` +
    `/home/user/video-use/reise-insider-subscribe-card-4k-transparent.webm`,
    { stdio: 'inherit' }
  );
  console.log('Done: reise-insider-subscribe-card-4k-transparent.webm');
